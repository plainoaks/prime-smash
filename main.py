...
class PrimeSmash(Widget):
    ...
    def fall_ball(self, ball):
        ball.value = randint(2,50)
        ball.center = (randint(ball.width/2,self.width-ball.width/2), self.height-ball.height/2)
        ball.velocity = Vector(0, -5)
        ...
        for ball in self.p_balls:
            if ball.collide_point(touch.x, touch.y):
                if isprime(ball.value):
                    ...
                    self.player.score -= ball.value
                    self.missed = True
                    self.label = Label(text = "miss", center = self.center)
            ...
            if ball1.collide_point(touch.x, touch.y):
                if touch.grab_current is self:
                    if isprime(ball1.value) == False:
                        ...
                        self.p_balls[-1].velocity = Vector(1.5,4)
                        self.p_balls[-2].velocity = Vector(-1.5,4)
                        self.add_widget(self.p_balls[-1])
...
class PrimeApp(App):
    def build(self):
        ...
        Clock.schedule_interval(game.isMissed, 3.0)
        return game
